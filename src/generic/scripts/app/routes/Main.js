...
import {
  ...
  getPresetFromData,
  handleGoogleAPI
} from 'utils/short-urls'

import { presetToPlaylistItem } from 'utils/riffs'
...
export default class Main extends Component {
  ...
  loadAndApplyPreset = preset =>
    preset.load.fork(logError, ({ default: fullPreset }) =>
  ...
  loadAdditionalViews = () => {
    require.ensure(
      ['containers/Instruments', 'containers/Sequences'],
      require => {
  ...
  updatePresetAndGetPlaylist = longURLs => {
    ...
    const sharedPresets = longURLs.traverse(Task.of, this.urlToPreset)

    sharedPresets.fork(logError, _sharedPresets => {
  ...
  setupSharedItemsAndUpdate = shareID => {
    this.setupSharedItems(shareID).fork(logError, audioPlaylist => {
  ...
  insertSoundsIntoPresetInstruments = preset => {
    preset.settings.instruments = preset.settings.instruments.map(i => {
      const inst = this.props.instruments.find(ins => ins.id === i.id)
      const sounds = getActiveSoundsFromHitTypes(i.predefinedHitTypes).map(
        sound => ({ ...inst.sounds.find(s => s.id === sound.id), ...sound })
      )
      return { ...i, sounds }
    })
  ...
  getViews = isMobileView => {
    const expandableTitleClass =
      'title-primary u-txt-large dropdown-icon-before u-curp u-no-select'
    return isMobileView ? (
      <SwipeableViews
        viewHeight={true}
        resistance={true}
  ...
  render = () => {
    const tabs = ['Player', 'Sequences', 'Instruments'].map((tabName, i) => (
      <div
        key={i}
        className={`nav-tab ${
          i === this.state.activePageIndex ? 'is-active' : ''
        }`}
        onClick={() => this.changeActivePageIndex(i)}
      >
        <div className="nav-tab__inner">{tabName}</div>
      </div>
    ))
