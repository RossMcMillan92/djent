            ...
            <div className={`note-panel ${isOn ? '' : 'note-panel--disabled'}`}>
                ...
                <div className="note-panel__amount-container">
                    ...
                    <div className="note-panel__btn-container">
                        <button className={`note-panel__btn note-panel__btn--${length.name} note-panel__btn--up`} title="Increase chance" onClick={e => this.onLengthAmountChange(e, 1)}></button>
                        <button className={`note-panel__btn note-panel__btn--${length.name} note-panel__btn--down`} title="Decrease chance" onClick={e => this.onLengthAmountChange(e, -1)}></button>
                    </div>
                </div>
                <div className="note-panel__checkbox-container u-flex-row u-flex-justify-center">
                    <div>
                        <div className={`toggle-input toggle-input--triplet u-txt-small u-txt-light ${length.isTriplet ? 'is-enabled' : ''}`} onClick={() => this.onModifierClick('isTriplet', 'isDotted')} >Triplet</div>
                        <div className={`toggle-input toggle-input--dotted u-txt-small u-txt-light ${length.isDotted ? 'is-enabled' : ''}`} onClick={() => this.onModifierClick('isDotted', 'isTriplet')} >Dotted</div>
                    </div>
                </div>
