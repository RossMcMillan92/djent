...
const getBufferFromInstrument = curry((context, instrument) => {
  ...
  const enabledSounds = newInstrument.sounds.filter(sound => sound.amount)
  ...
  return Task((rej, res) => {
    ...
    enabledSounds.forEach((sound, i, sounds) =>
      getBufferFromURL(context, sound.path).fork(rej, buffer =>
        onComplete(buffer, sound, sounds)
      )
    )
  })
})
...
const playSoundSimple = curry((context, buffer) =>
  IO(() => playSound(context, buffer, 0, buffer.duration, 1))
)
